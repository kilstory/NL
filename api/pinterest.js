export default async function handler(req, res) {
  ...
  try {
    ...
    // Extract pin image URLs natively
    const match = html.match(/https:\/\/i\.pinimg\.com\/[^\"]+\.jpg/g);
    
    if (match && match.length > 0) {
      // Remove duplicates
      const uniqueUrls = [...new Set(match)];
      // Filter out small thumbnails if possible (Pinterest often has 236x, 474x, 736x, originals)
      // Attempt to upgrade to 736x or originals
      let bestUrl = uniqueUrls[0];
      
      // Look for a 736x or originals version from the matches to ensure high quality
      const highQuality = uniqueUrls.find(u => u.includes('736x') || u.includes('originals'));
      if (highQuality) bestUrl = highQuality;
